...
@section('content')
    <h4 class="center">Загрузки талонов</h4>
    <p><a href="{{ route('admin.fuel_files.create') }}" class="btn waves-effect waves-light"><i class="material-icons left">add_circle</i> Добавить</a></p>
            ...
            <tbody>
                @foreach($items as $item)
                    <tr>
                        ...
                        <td>{{ $item->is_imported ? 'Да' : 'Нет' }}</td>
                        <td>{{ $item->rows_imported }}</td>
                        <td>{{ $item->total_rows }}</td>
                        <td>{{ $item->user->name }}</td>
                        <td>{{ $item->created_at->format('Y-m-d H:i:s') }}</td>
                        <td><a href="{{ route('admin.fuel_files.edit', $item->id) }}" class="btn btn-small waves-effect waves-light"><i class="material-icons">edit</i></a></td>
